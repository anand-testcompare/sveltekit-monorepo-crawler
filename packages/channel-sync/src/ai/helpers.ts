...
export const classifyComment = (data: { comment: string; videoSponsor: string | null }) => {
	...
	return ResultAsync.fromPromise(
		...
		(error) => new Error(`Failed to classify comment with prompt: ${error}`)
	).map((result) => result.object);
};

export const getSponsor = (data: { sponsorPrompt: string; videoDescription: string }) => {
	...
	return ResultAsync.fromPromise(
		...
		(error) => new Error(`Failed to get sponsor with prompt: ${error}`)
	).map((result) => result.object);
};
