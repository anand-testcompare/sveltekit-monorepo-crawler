<script lang="ts">
	...
	const fullData = $derived(channelId ? await remoteGetLast7VideosByViews(channelId) : null);
...
{:else}
	<p class="text-xl font-bold text-muted-foreground">Most recent 7 videos</p>
	<div class="overflow-hidden rounded-lg border border-border bg-card">
		<table class="w-full">
			<thead>
				<tr class="border-b border-border">
					<th class="px-6 py-3 text-left text-sm font-medium text-muted-foreground"> Thumbnail </th>
					...
					<th class="px-6 py-3 text-left text-sm font-medium text-muted-foreground"> Days Ago </th>
